...
- Wants programmatic SEO landing pages for each image conversion pair as nested dynamic routes under `/image-converter` (e.g., `/image-converter/heic-to-jpeg`, `heic-to-png`) using TanStack Start file-based router (`$conversion`) with per-page SEO and prerender via `prerender.crawlLinks` to capture long-tail conversion search traffic. Confidence: 0.85
